...
local p = swin:Toggle("Survivor ESP", {
    flag = "suresp"
}, function(flag)
...
swin:Toggle("Shark TP Farm", {
    flag = "tpshark"
}, function(flag)
end)

swin:Toggle("Kill Survivors", {
    flag = "ksur"
}, function()
end)
                ...
                if game.Players.LocalPlayer.Team.Name == "Shark" then
                    for i, v in pairs(game.Players:GetChildren()) do
                        if v.Team.Name == "Survivor" then
                            ...
                            workspace.Sharks["Shark" .. game.Players.LocalPlayer.Name]:MoveTo(
...
function checkGun()
    if not game.Players.LocalPlayer.PlayerGui.WeaponDisplay:FindFirstChild("Frame") then
    ...
    pcall(function()
        ...
        while wait() do
            if swin.flags.afk then
                ...
                loc.CameraMode = Enum.CameraMode.Classic
            end
        end
    end)
end)

swin:Toggle("No Barriers", {
    flag = "barrier"
}, function(flag)
    if flag then
            ...
            for i, v in pairs(game.CoreGui.esp.sharks:GetChildren()) do
                if v then
                    if v:IsA("BillboardGui") then
                        ...
                        for i = 1, #oo do
                            if workspace.Sharks:FindFirstChild(tostring(v.Name)) and
                                game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                                local texts = math.ceil((v.Adornee.Position -
                        ...
                        pcall(function()
                            if game.Players[v.Name].Team.Name == "Survivor" then
                                if game.Players.LocalPlayer.Team.Name == "Shark" then
                                    ...
                                    local texts = math.ceil((v.Adornee:FindFirstChild("HumanoidRootPart").Position -
                                                                workspace.Sharks["Shark" .. LocalPlayer]:WaitForChild(
...
spawn(function(rainbow)
    while wait() do
        ...
        game.CoreGui.ScreenGui.Container.Sharkbite.Underline.BackgroundColor3 =
            Color3.fromHSV(tick() * 24 % 255 / 255, 1, 1)
        wait()
    end
end)
