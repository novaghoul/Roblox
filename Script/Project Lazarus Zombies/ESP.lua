...
function hitBox()
    for _, o in pairs(wp.Baddies:GetChildren()) do
        if o:WaitForChild("HeadBox") then
            wait(0.1)
            createHitBox(o)
        end
    end

    wp.Baddies.ChildAdded:connect(
        function(m)
            if m:WaitForChild("HeadBox") then
                wait(0.1)
                createHitBox(m)
            end
        end
